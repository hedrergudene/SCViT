...
# Model
class ViT_classifier(tf.keras.layers.Layer):
    def __init__(self,
                 depth:int,
                 depth_te:int,
        ...
        # Output
        self.Tube = []
        self.Tube.append(tf.keras.layers.Flatten())
        self.linear_list = linear_list
        for i in range(len(linear_list)):
            self.Tube.append(tf.keras.layers.Dense(self.linear_list[i]))
            self.Tube.append(tf.keras.layers.Dropout(self.linear_drop))
